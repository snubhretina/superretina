...
from common.log import SummaryWriterAvg
...
def train_model(model, optimizer, dataloaders, device, num_epochs, train_config):
    ...
    Train_SummaryWriter = SummaryWriterAvg(log_dir=train_config['summary_path'],
                                       flush_secs=10, dump_period=25)
        ...
        for phase in phases:
            ...
            if 'val' in phase:
                if epoch % model_save_epoch == 0:
                    ...
                    model_save_path[:-4] + "_{}.pth".format(epoch)
                continue
            ...
            Train_SummaryWriter.add_scalar(tag=f'Detection Losses/detector_loss',
                                       value=print_detector_loss.item(),
                                       global_step=epoch)
            Train_SummaryWriter.add_scalar(tag=f'Detection Losses/descriptor_loss',
                                       value=print_descriptor_loss.item(),
                                       global_step=epoch)
            Train_SummaryWriter.add_scalar(tag=f'num_learned_pts',
                                       value=num_learned_pts / num_input_with_label,
                                       global_step=epoch)
            Train_SummaryWriter.flush()
